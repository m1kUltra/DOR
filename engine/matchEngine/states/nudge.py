...
AFTER ="nudge.after_conversion"

NUDGE_TAGS = { QUICK_TAP, SLOW_TAP, CONVERSION, PEN_PUNT, FREE_KICK, AFTER }
...
def maybe_handle(match, tag, loc, ctx) -> bool:
    ...
    if tag == AFTER:
        ...
        conversion_transit(match, to, success)
        match.ball.update(match) 
        return True
    if tag == CONVERSION:
        
        
        side = conversion_now(match, to=to)
