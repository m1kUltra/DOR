...
def assign_primary_tackler(ball_xy: Tuple[float,float], defenders: List[object], ctx) -> Optional[int]:
    ...
    for d, d2, dx, _ in cands:
        if d2 <= 25*25 and (dx * att_dir) <= 10.0:
            return d.sn
    return cands[0][0].sn if cands else None

def shape_defensive_line(ball_xy: Tuple[float,float], defenders: List[object], ctx, style="drift") -> Dict[int, Tuple[float,float,float]]:
    bx, by = ball_xy
    my_dir = ctx["teams"][defenders[0].team_code]["dir"] if defenders else +1.0
    line_x = bx + (LINE_DEPTH * my_dir)
    ...
    for d in defs:
        ...
        # drift: slightly wider than the ball on your side
        if style == "drift" and ty > by:
            ty += 0.5
        if style == "drift" and ty < by:
            ty -= 0.5
        # clamp and remember clamped value for next step
